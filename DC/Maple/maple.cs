...
namespace DC4Ever
{
	public unsafe class maple
	{
		...
		public static void MapleEnable(uint data)
		{
			*mem.MAPLE_ENABLE = data;//??? what is that ??
		}

		public static void MapleDMAState(uint data)
		{
			//Start Maple DMA transfer
			if ((data & 0x1)!=0)
			{
				uint addr = *mem.MAPLE_DMAADDR;
				bool last = false;
				while (last != true)
				{
					uint header_1 = mem.read(addr, 4);
					uint header_2 = mem.read(addr + 4, 4) &0x1FFFFFE0;
					if (header_1 == 0)//this must not hapen ...
					{
						mem.WriteLine("Maple ; Header1 coruption,ending transfer");
						mem.write(header_2, 0xFFFFFFFF, 4);//not conected;
						break;
					}
					...
					else
					{
						...
						{
							p_data[i] = mem.read(addr + 8 + (i << 2), 4);
						}
				...
				*mem.MAPLE_STATE = 0;//finished :P
				intc.RaiseInterupt(sh4_int.holly_MAPLE_DMA);
			}
...
//			MapleDMAFinished = true;
		...
		public static void MapleReset2(uint data)
		{
			//just ingore the write..
		}
		public static void MapleReset1(uint data)
		{
			//just ingore the write..
		}
		public static void MapleSpeed(uint data)
		{
			//just ingore the write..
		}

		public static void InitMaple()
		{
	...
	public unsafe class DcJoy : MapleDevice
	{
		...
		public void GotData(uint header1,uint header2,uint*data,uint datalen)
		{
			...
			if (recadr != 0x20)
			{
				mem.write(header2, 0xFFFFFFFF, 4);//not conected
				return;
			}
			switch (command)
			{
				...
				case 1:
					ptr_out = header2;
					//header
					mem.write(ptr_out,(uint)(0x05 | //response
								(((ushort)sendadr << 8) & 0xFF00) |
								((((recadr == 0x20) ? 0x20 : 0) << 16) & 0xFF0000) |
								(((112/*size*//4) << 24) & 0xFF000000)),4); ptr_out += 4;
					//caps
					//4
					mem.write(ptr_out, (1 << 24), 4); ptr_out += 4;

					//struct data
					//3*4
					mem.write(ptr_out, 0, 4); ptr_out += 4;
					mem.write(ptr_out, 0, 4); ptr_out += 4;
					mem.write(ptr_out, 0, 4); ptr_out += 4;
					//1
					mem.write(ptr_out, 0, 1); ptr_out += 1;
					//1
					mem.write(ptr_out, 0, 1); ptr_out += 1;
					//30
					for (uint i = 0; i < 30; i++)
					{
						if (i<strName.Length)
						{
							mem.write(ptr_out + i, (byte)strName[(int)i], 1);
						}
					}
					ptr_out += 30;
					...
					for (uint i = 0; i < 30; i++)
					{
						...
						{
							mem.write(ptr_out + i, (byte)strBrand[(int)i], 1);
						}
					}
					ptr_out += 60;

					//2
					mem.write(ptr_out, 0, 2); ptr_out += 2;
				...
				case 9:
					...
					//header
					mem.write(ptr_out, (uint)(0x08 | // data transfer (response)
								(((ushort)sendadr << 8) & 0xFF00) |
					...
					//struct data
					//2
					mem.write(ptr_out, frmmain.kcode, 4); ptr_out += 2;
